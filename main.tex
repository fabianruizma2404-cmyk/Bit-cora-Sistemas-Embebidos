...
\begin{itemize}
    \item \textbf{Arduino.h}: Librería principal del framework Arduino. 
    Proporciona las funciones básicas para la interacción con el hardware, tales como 
    \texttt{pinMode()}, \texttt{digitalWrite()}, \texttt{analogRead()}, \texttt{delay()} y \texttt{millis()}. 
    Es indispensable para cualquier proyecto en PlatformIO que utilice el framework Arduino.

    \item \textbf{Serial}: Módulo de comunicación serial incluido en el núcleo de Arduino. 
    Permite enviar y recibir datos entre la placa y el ordenador, facilitando la depuración y el monitoreo de valores 
    mediante funciones como \texttt{Serial.begin()}, \texttt{Serial.print()} y \texttt{Serial.println()}.

    \item \textbf{Funciones de tiempo}: Integradas en el núcleo de Arduino, 
    permiten gestionar retardos y medir intervalos de tiempo. 
    \texttt{delay()} se utiliza para pausar la ejecución en milisegundos, 
    mientras que \texttt{millis()} devuelve el tiempo transcurrido desde que la placa fue encendida.

    \item \textbf{Entradas y salidas digitales/analógicas}: 
    Conjunto de funciones que permiten la interacción directa con los pines de la placa. 
    \texttt{pinMode()} configura un pin como entrada o salida, 
    \texttt{digitalWrite()} establece un nivel lógico en un pin digital, 
    y \texttt{analogRead()} obtiene el valor de un pin analógico, fundamental para leer la señal del sensor HW-080.
\end{itemize}
